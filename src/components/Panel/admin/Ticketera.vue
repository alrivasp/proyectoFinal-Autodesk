              ...
              <tbody>
                <tr class="shadow" v-for="tkt in showTickets" :key="tkt.id">
                  ...
                  <td
                    v-if="
                      calculationSla(
                        tkt.data.dateTimeStart,
                        tkt.data.sla,
                        tkt.data.dateTimeEnd
                      )
                    "
                  >
                    <p
                  ...
                  <td>
                    <ModalGestion
                      :ticketSelected="tkt"
                      :technical="getTechnical(tkt.data.technicalId)"
                    />
                    <ModalReasign
                      :ticketSelected="tkt"
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("Ticketera", ["getTicketsAdmin"]),
    showTickets() {
      if (this.search.length < 1 && this.getTicketsAdmin.length < 1) {
        return [];
      }
      if (this.search.length < 1 && this.getTicketsAdmin.length > 0) {
        return this.getTicketsAdmin;
      }
      const result = this.getTicketsAdmin.filter(
