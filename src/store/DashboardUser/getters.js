...
export default {
    ...
    getResultsInProgress({ tickets }, { }, { currentUser }) {
        let resume = [] //{ date:'', count: 0 }
        let filteredId = tickets.filter(t => t.data.userId == currentUser.id)
        let filteredStatus = filteredId.filter(t => t.data.ticketStatus === 'En progreso');
        filteredStatus.sort((a, b) => a.data.dateTimeStart > b.data.dateTimeStart);
    ...
    getBeaten({ tickets }, { }, { currentUser }){
        ...
        let result = 0;
        for (const key in filteredId) {
            if (!mixins.methods.calculationSla(filteredId[key].data.dateTimeStart, filteredId[key].data.sla)){ result += 1 }
        }
        return result
    },
